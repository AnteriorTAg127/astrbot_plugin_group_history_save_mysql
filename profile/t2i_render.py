...
``ValueError`` 由 WebAPI 端点转 502 明确报错。日志统一 ``[Profile]`` 前缀。
...
class ProfileT2IRenderer:
    ...
    async def render_from_dict(self, data: dict) -> bytes:
        ...
        try:
            ...
            logger.warning(f"[Profile] 导出模板数据组装失败: {e}", exc_info=True)
    ...
    async def _render_two_rounds_bytes(self, tmpl: str, data: dict) -> bytes | None:
        ...
        for round_no, options in rounds:
            start = time.monotonic()
            logger.info(
                f"[Profile] T2I 导出第 {round_no} 轮渲染开始"
                f"（{options['type']}，超时 {options['timeout']}ms）"
            )
            try:
                ret = await self.star.html_render(
                ...
                logger.warning(
                    f"[Profile] T2I 导出第 {round_no} 轮渲染异常"
            ...
            if self._validate_image(ret):
                ...
                if img_bytes:
                    return img_bytes
            logger.warning(
                f"[Profile] T2I 导出第 {round_no} 轮渲染结果校验失败，耗时 {cost:.1f}s"
            )
        logger.warning("[Profile] T2I 导出两轮渲染均失败")
        return None
