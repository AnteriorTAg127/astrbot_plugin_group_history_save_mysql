...
### 补库工作原理（v0.6.0 新增，v0.6.1 完善）

1. **触发时机**：插件加载/重载、MySQL 初始化成功后，后台自动发起（不阻塞插件启动，失败仅记日志）；
   **补库开始前实时消息进入缓冲**（门控缓冲，v0.6.1），补库期间不落库、补库完成后统一带去重写入
2. **范围**：白名单模式取 group_config 白名单中 `enabled=1` 的群；all_mode 全局记录模式白名单无意义，
   改以 `chat_history` 有数据的群为清单（补库只补缺口，无数据的群拉协议端也是空），逐群串行处理
3. **窗口**：起点取「上次插件卸载时间（`last_terminate_time`，停机缺口）与 `now - backfill_hours`」的较新者，
   拉取量收窄到停机缺口；无记录/非法回退 `backfill_hours`（v0.6.1）
4. **拉取**：经 aiocqhttp 协议端 `get_group_msg_history` 按 `message_seq` 多轮翻页（单群总上限 =
   单轮上限 × 最大轮数，默认 200 条 × 5 轮 = 1000 条，两者可经配置自定义；轮间 0.3s、超时 15s；
   首轮失败整群跳过，后续轮失败用已拉到的部分；翻页中本轮最旧消息已出窗口，或本轮与已记录消息
   **多数相同**（到达已记录边界，v0.6.1）即提前停止拉取）
5. **窗口过滤**：仅保留 `timestamp >= 窗口起点` 的消息；空 `message_id` 消息补库侧跳过
   （由实时缓冲单源覆盖，v0.6.1）
6. **去重入库**：按 `message_id` 批量比对 `chat_history` 已存在记录（跳过）；图片 URL 按
   `(group_id, image_url)` 比对 `image_records` 已存在记录（跳过），单条消息内重复 URL 只入一次；
   **按最旧→最新顺序写入**，保持自增 id 与时间单调（v0.6.1）
7. **入库**：文本消息写入 `chat_history`（message_type 按有无图片 text/mixed，at_list/reply_id/真实
   时间戳透传），图片 URL 逐条写入 `image_records`；单条失败记 error 不中断整群
8. **收尾（v0.6.1）**：全部群补库完成后，把补库期间缓冲的实时消息**带去重**写入（跳过补库已写入的
   message_id / 图片 URL），再触发快照启动回填，使回填历史纳入快照层
9. **汇总日志**：每群完成与全部完成均输出 `[HistorySave] 重载自动补库` 汇总（拉取/新增文本/新增图片/
   跳过计数），可据日志核对补库情况
