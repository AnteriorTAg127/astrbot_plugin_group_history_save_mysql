...
- **NapCat 兼容（message_seq=0）**：首轮拉取不传 `message_seq`（协议端按最新开始），
  NapCat 等对 `message_seq=0` 报「消息0不存在」不再导致首轮失败整群作废（补库与
  总结 OneBot 拉取路径同步修复）
- **解析失败误报修复**：视频/表情/语音/转发等**无 text/image 段**的消息是正常历史内容，
  不再计入「解析失败」warning（原实现把这类消息误报为解析失败、导致群级「拉取 0 条」）；
  仅含文本/图片但 time 缺失或结构异常才计数
