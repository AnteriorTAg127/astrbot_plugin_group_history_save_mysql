...
实时/补库并发竞态致重复行、文本拼接口径分叉、快照回填时序、窗口过滤效率等缺陷，并将
补库窗口收窄到停机缺口。MySQL 零 schema 变更，无新增用户配置。

### Fixed

- **补库门控缓冲（关键）**：补库进行期间实时消息先缓冲不落库，补库全部写完后再带去重
  flush（按群批量查已存在 message_id / 图片 URL），两条写路径由并发改串行，消除竞态
  重复行（不引入唯一索引）
- **多轮翻页真实生效（可配置）**：单轮上限 / 最大轮数改为插件设置（默认单轮 200 × 最多 5 轮，
  单群总上限 = 单轮 × 轮数），每轮请求量按剩余计算，短页判定对齐实际请求数——协议端单次硬限
  （约 200 条）时仍能多轮翻页累计
- **重叠边界停止**：翻页前预加载该群最近已记录消息的 ID / 文本内容，每轮拉取后与已记录消息
  **多数相同**即停止翻页（到达已记录边界），随后去重入库——快速重载 / 停机短时不空拉整窗
- **插入顺序最旧→最新**：补库不再按翻页序（最新→最旧）落库，保持 chat_history 自增 id
  与时间单调（stats 的 MAX(id)=最新消息 昵称查询假设不再被破坏）
- **空 message_id 消息补库侧跳过**：空 id 消息只来自实时缓冲单源路径，不再每次重载重复入库
- **文本拼接统一 `\n`**：补库解析与实时路径 `"\n".join` 口径一致，同一消息两条路径落库内容不再分叉
- **快照回填时序**：补库收尾（含缓冲 flush）后链式触发快照启动回填，回填历史纳入快照层
- **窗口过滤提前终止**：翻页循环内发现本轮最旧消息已在窗口外即停止拉取，安静群不再拉满
- **缺 time 键不再静默丢弃**：解析缺 time / 非数值记 warning（带 message_id），解析失败条数汇总 warning
- **NapCat 兼容（message_seq / 缓存空）**：首轮拉取不传 `message_seq`（标准 OneBot 省略
  即从最新开始；NapCat 源码确认省略与传 0 等效——都走「最新消息视图」，群缓存为空时返回
  空列表报「消息不存在」）。首轮失败由「整群作废」改为「降级跳过该群」（记 warning 保留
  traceback），群缓存空 / 无新消息不再刷「补库失败」（补库与总结 OneBot 拉取路径同步修复）
- **解析失败误报修复**：视频/表情/语音/转发等**无文本无 http 图片**的消息是正常历史内容，
  不再计入「解析失败」warning（原实现把这类消息误报为解析失败、导致群级「拉取 0 条」；
  图片 URL 为本地路径——NapCat 历史图片常见——的消息同样正常跳过）；仅含非空文本或
  http 图片链接但 time 缺失 / 结构异常才计数
- **backfill_hours 校验统一**：`int(str(value))` 拒绝浮点 / 布尔（不再静默截断）
- **批内图片 URL 去重收窄为单消息内**：不同消息共用同一 URL 各写一行，对齐实时统计口径

### Changed

- **补库改为按群消息触发**：插件重启后某群第一条新消息到达时触发该群补库，用该消息真实
  `message_seq` 作 `get_group_msg_history` 起点往前翻停机缺口（NapCat 走 `getMsgHistory`
  正式历史，不依赖 aio 最新视图，解决「消息不存在」缓存空误报）；每群在本重启周期内只补库
  一次，补库期间该群新消息门控缓冲、完成后带去重 flush；不再在加载时对所有群批量补库
- **补库窗口收窄为停机缺口**：插件卸载时持久化 `last_terminate_time`，加载时窗口起点取
  「上次卸载时间与 now-backfill_hours 的较新者」，拉取量收窄到停机缺口（无记录 / 非法
  回退 backfill_hours）
- **缓冲容量**：`MessageSaver` 缓冲 maxlen 2000 → 5000；terminate 取消补库任务时收尾链
  仍执行带去重 flush，保全缓冲数据
- **pool.py 循环导入消除**：连接池行为常量移至 pool.py，`core.db_mysql.pool` 可被直接导入
- **补库翻页参数可配置**：`plugin_settings` 新增 `backfill_round_cap`（默认 200，夹取 [1,5000]）/
  `backfill_max_rounds`（默认 5，夹取 [1,50]），Web「设置」页可改；`db_mysql` 新增
  `get_recent_messages`（按群取最近已记录消息的 ID/内容，供重叠边界比对）
- 版本升至 0.6.1，`@register` 与 metadata.yaml 同步

### Notes

- 存量数据与配置完全向后兼容；MySQL 零 schema 变更
- 回归保障：v0.6.1 新增 26 例离线测试覆盖（翻页 cap 分层 / 重叠边界停止 / 插入顺序 / 门控缓冲
  去重 flush / 空 id 跳过 / 窗口提前终止 / backfill_hours 校验 / 循环导入 / 版本号），详见
  `开发/v0.6.1/test/test_0.md`
...
「消息 + 图片 × 小时/日/月」三段式预计算快照体系，数据分析**群级查询**迁移到快照读数
（发言人/个人维度保持实时 SQL），Web 概览「最近 7 天趋势」改用快照口径（消除
`image_records` 滚动清理导致的趋势抖动），并补齐快照优化三件套：启动历史回填、
宕机缺档补偿、强制刷新限流。不涉及 LLM，无新增 pip 依赖，MySQL 零 schema 变更。

### Added

- **分段快照体系**：SQLite config.db 新增 6 张快照表——`msg_stats_hourly`（每小时每群消息总数）/
  `msg_stats_hourly_top`（每小时每群发言人 Top K 含昵称）/ `msg_stats_daily`（每群每日消息总数，
  全量不受 Top K 截断）/ `msg_stats_monthly`（每群每月消息总数，month="YYYY-MM"）/
  `image_stats_daily`（每群每日图片总数）/ `image_stats_monthly`（每群每月图片总数）；
  全部沿用 UPSERT 覆盖写语义，快照行只增盖不删
- **小时快照任务双源化**：每小时第 5 分钟起同时聚合**上一个完整小时**的消息（`chat_history`）
  与图片（`image_records`），UPSERT 四张 hourly 表（消息群级 + Top K、图片群级 + Top K）；
  消息 Top K 与图片共用 `stats_image_top_k`
- **新增每日/每月快照任务**：每日 00:15 起聚合**上一完整自然日**的每群消息/图片总数写入两张
  daily 表（内存日期戳去重，成功后盖章，失败自动补跑）；每月分钟 ≥ 25 且上一自然月未盖章时
  聚合上一自然月写入两张 monthly 表；daily 表只存**完整日**、monthly 表只存**完整月**，
  「今日实时」永远由小时快照的强制刷新保证
- **快照淘汰机制**：每日日快照任务成功后顺带执行一次（启动回填后也执行一次）——小时层三张表
  （`msg_stats_hourly(_top)` / `image_stats_hourly`）保留 **7 天**；`image_stats_hourly_top`
  保留 **31 天**（保护近 30 天报告的个人图片口径）；日层两张表保留**上月 + 本月**；
  月层两张表**永不淘汰**；淘汰线与三层归并的地平线（上月 1 日）严格对齐
- **启动历史回填与宕机缺档补偿**：MySQL 初始化成功后后台执行一次，全部为**窗口批量重聚合**
  （每层一条 GROUP BY SQL、幂等 UPSERT，非逐小时循环）——小时层一次性聚合
  `[7 天前 00:00, 当前整点)`，同时完成「首次安装近 7 天历史回填」与「宕机错过的整点缺档补偿」；
  日层一次性聚合 `[上月 1 日, 今天 00:00)`；月层按**持久游标**增量补齐
  （plugin_settings 记录已覆盖到的月份，首次安装游标缺省 = 全历史，重启不重复聚合）；
  异常仅记日志不阻断启动，下次重启窗口层自动重跑（幂等）
- **强制刷新限流**：`refresh_current_hour` 泛化为消息 + 图片双源并增加节流——
  **60 秒内至多真正执行一次**，期间重复调用直接跳过；高频统计请求（Web 面板反复刷新、
  多群推送串行 build）不再反复触发 MySQL 聚合；刷新失败不阻断统计主流程（契约不变）

### Changed

- **数据分析群级查询迁快照**：`build_stats` 总消息数 / 每日趋势 / 群排行消息数改由预计算快照供数——
  **三层归并**（月层取范围内完整包含的月份，剩余日期逐日取数：该日有 hourly 行则用 hourly 求和、
  否则用 daily 行，存在性优先，无重叠无空洞），口径精确、无 Top K 截断误差；
  自定义区间切断旧月中间等**快照不可服务**条件或快照层异常时**自动整体回退现有实时 SQL 路径**，
  不报错、行为对用户透明；活跃成员数、首/末条消息时间、发言人排行、个人维度全部、
  24h/星期分布与派生量保持实时 SQL 不变
- **图片注入口径升级**：总图片数与群排行 `image_count` 改用图片三层归并（同消息算法）；
  发言人 `image_count` 仍取 `image_stats_hourly_top`（Top K 口径，保留期 31 天）
- **Web 概览「最近 7 天趋势」改快照口径**：`GET /stats/daily` 在 `days ≤ 31` 时由日快照供数
  （今天 = 小时快照求和），供数前触发一次限流强制刷新保证「今天」柱子实时；
  响应结构不变，前端零改动；`days > 31`（API 允许至 90）或快照不可用 → 现有 MySQL 实时路径不变
- **`stats_image_top_k` 语义扩展**为「快照 Top K（图片/消息共用）」：键名、取值范围（1–100，
  默认 20）、校验与存储均不变，仅 Web UI 标签文案微调
- 版本升至 0.5.5，静态引用 `?v=0.5.5`

### Fixed

- **概览趋势图片口径抖动**：Web 概览「最近 7 天趋势」每日合计 = `chat_history` 当日消息数 +
  `image_records` 当日图片数，而 `image_records` 按 `image_retention_days`（默认 3 天）滚动清理——
  最近几天的柱子清理前含图片、清理后图片部分消失，同一天数据在清理前后不一致，趋势「变来变去」。
  本版改由快照供数（快照行在清理前已固化、只增盖不删），趋势稳定

> **备注（个人图片数口径变化）**：`image_stats_hourly_top` 改为 31 天淘汰后，超 31 天范围的
> 发言人/个人图片数归 0（此前为永久保留，小时层 Top K 淘汰所致）；群级图片总数不受影响
> （日/月快照兜底）。上线前 `image_records` 已被清理的天数无法追溯回填历史图片数
> （旧月图片月快照自然为 0 行）。
